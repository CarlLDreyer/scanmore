package com.example.scanmore.Payment.PaymentUtils;
...
public class CreditCardUtils {
    ...
    public static boolean isValid(String cardNumber) {
        if (!TextUtils.isEmpty(cardNumber) && cardNumber.length() >= 4)
            if (getCardType(cardNumber) == VISA && ((cardNumber.length() == 16 || cardNumber.length() == 19)))
                return true;
            else if (getCardType(cardNumber) == MASTERCARD && cardNumber.length() == 19)
                return true;
        return false;
    }
